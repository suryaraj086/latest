...
public class TempStore {
	...
	Map<Integer, Integer> existingCustomer = new HashMap<Integer, Integer>();
...
//booking
	...
	public String onlinePortalPayment(UserDetails user, int tokenNum) {
		...
		if (user.getUserId() == 0) {
			userVal = userNumber();
			user.setUserId(userVal);
			existingCustomer.put(userVal, tokenNum);
			return "Payment Succesful and user id is " + userVal;
		}
	...
	public void balanceSetter(int token, int balance) {
		UserDetails user = userMap.get(token);
	...
	public int freeFloor(VehicleSize type) throws Exception {
		for (int i = 1; i < emptySpots.size(); i++) {
			if (emptySpots.get(i).get(type).size() != 0) {
				return i;
			}
		}
		throw new Exception("Fully Occupied");
